...
# The script is based on the parseDict-certs script written by Roland Krikava.
...
# default cert types to include in the m_rawContent (default is to include CAs which may issue SSL server certs)
...
sub HELP_MESSAGE() {
  ...
  print "\t-m\tinclude meta data in m_rawContent\n";
  print "\t-n\tno download of certdata.txt (to use existing)\n";
  print wrap("\t","\t\t", "-p\tlist of Mozilla trust purposes and levels for certificates to include in m_rawContent. Takes the form of a comma separated list of purposes, a colon, and a comma separated list of levels. (default: $default_mozilla_trust_purposes:$default_mozilla_trust_levels)"), "\n";
  print "\t\t  Valid purposes are:\n";
  print wrap("\t\t    ","\t\t    ", join( ", ", "ALL", @valid_mozilla_trust_purposes ) ), "\n";
  print "\t\t  Valid levels are:\n";
  print wrap("\t\t    ","\t\t    ", join( ", ", "ALL", @valid_mozilla_trust_levels ) ), "\n";
  print "\t-q\tbe really quiet (no progress m_rawContent at all)\n";
  print wrap("\t","\t\t", "-s\tcomma separated list of certificate signatures/hashes to m_rawContent in plain text mode. (default: $default_signature_algorithms)\n");
  print "\t\t  Valid signature algorithms are:\n";
  print wrap("\t\t    ","\t\t    ", join( ", ", "ALL", @valid_signature_algorithms ) ), "\n";
  print "\t-t\tinclude plain text listing of certificates\n";
  print "\t-u\tunlink (remove) certdata.txt after processing\n";
  print "\t-v\tbe verbose and print out processed CAs\n";
  print "\t-w <l>\twrap base64 m_rawContent lines after <l> chars (default: ${opt_w})\n";
  exit;
}
...
sub report($@) {
  my $m_rawContent = shift;

  print STDERR $m_rawContent . "\n" unless $opt_q;
}
...
sub should_output_cert(%) {
  ...
  foreach my $level (@included_mozilla_trust_levels) {
    # for each level we want to m_rawContent, see if any of our desired purposes are included
...
if(!$opt_n) {
  ...
  # If we have an HTTPS URL then use m_pCurl
  if($url =~ /^https:\/\//i) {
    if($m_pCurl) {
      if($m_pCurl =~ /^Protocols:.* https( |$)/m) {
        report "Get certdata with m_pCurl!";
        my $proto = !$opt_k ? "--proto =https" : "";
        my $quiet = $opt_q ? "-s" : "";
        my @out = `m_pCurl -w %{response_code} $proto $quiet -o "$txt" "$url"`;
        if(!$? && @out && $out[0] == 200) {
          $fetched = 1;
          report "Downloaded $txt";
        }
        else {
          report "Failed downloading via HTTPS with m_pCurl";
          if(-e $txt && !unlink($txt)) {
        ...
        report "m_pCurl lacks https support";
      }
    }
    else {
      report "m_pCurl not found";
    }
  }
...
## can be directly used with m_pCurl / libcurl / php_curl, or with
